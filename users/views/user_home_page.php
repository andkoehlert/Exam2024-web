...
<h1>Your Page</h1>
<p>Discover all your posts below:</p>

<div class="posts-container">
    
    <?php if (!empty($postsUser)): ?>
    
        <?php
        require_once __DIR__ . '/../controllers/commentController.php'; 

         foreach ($postsUser as $post):
            // Ensure $postId is set before using it
            $postId = $post['post_id'];
            ...
            $userLiked = false; // Logic to determine if the user liked the post
           
            // Fetch comments for this post using the CommentController
            $commentController = new CommentController($pdo);  // Assuming $pdo is available
     ...
     ?>
       ...
       <div id="post-<?php echo $postId; ?>" class="post-card">
            ...
            <?php if ($isMostLiked): ?>
                <p class="on-fire">🔥 This post is on fire! 🔥</p>
            <?php endif; ?>
        </div>
...
if (($post['user_id'] == $userId) || ($_SESSION['role'] === 'admin')) {
    ...
    <!-- Delete button that triggers the deletePost function -->
...
<?php endif; ?>
                    ...
                    <!-- Delete button, only visible if the user owns the comment or is an admin -->
                    <?php if ($comment && ($comment['user_id'] == $userId || $_SESSION['role'] == 'admin')): ?>
                        <!-- Delete button inside a form to submit a POST request -->
    ...
    <?php endif; ?>
                        ...
                        <!-- Add comment form -->
      <form id="btnRefresh" class="comment-form" method="POST" action="comments/submit">
